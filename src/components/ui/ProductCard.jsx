...
export default function ProductCard({ product }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 12 }}
      whileHover={{ y: -2, boxShadow: '0 12px 36px rgba(28,61,121,0.12)' }}
      transition={{ duration: 0.22 }}
      className={`flex flex-col bg-white${modalEnabled ? ' cursor-pointer' : ''}`}
      style={{ border: '1px solid #DDDDE8', boxShadow: '0 1px 4px rgba(28,61,121,0.06)' }}
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-1 px-3 pt-3 pb-1">
        <span className="text-[9px] tracking-[0.25em] uppercase" style={{ color: '#1C3D79' }}>
          {product.category}
        </span>
        <h3 className="font-normal leading-snug" style={{ ...SERIF, fontSize: '1.05rem', color: '#0D1B3E' }}>
          {product.name}
        </h3>
      ...
      <div
        ...
        style={{ borderTop: '1px solid #DDDDE8' }}
        onClick={(e) => e.stopPropagation()}
      >
        <span className="font-light" style={{ ...SERIF, fontSize: '1.1rem', color: '#0D1B3E', letterSpacing: '0.02em' }}>
        ...
        {qty === 0 ? (
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{ border: '1px solid #DDDDE8', color: '#0D1B3E', backgroundColor: 'white' }}
            >
              −
            </button>
            <span className="font-black text-sm w-4 text-center" style={{ color: '#0D1B3E' }}>{qty}</span>
            <button
              onClick={() => updateQty(product.id, 1)}
              className="w-6 h-6 flex items-center justify-center font-bold text-sm"
              style={{ backgroundColor: '#EC1C23', color: '#FFFFFF' }}
