      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            style={{ width: 'min(100vw, 380px)', backgroundColor: '#0D1B3E' }}
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid #1A2E5A' }}>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
              {items.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-3 py-16">
                  ...
                  <p className="text-[11px] tracking-widest uppercase" style={{ color: '#3D5A99' }}>
                    Tu carrito está vacío
                  </p>
                </div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="flex items-center gap-3" style={{ borderBottom: '1px solid #1A2E5A', paddingBottom: '1rem' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs" style={{ color: '#8A9DC0' }}>{currency}{(item.price * item.qty).toFixed(2)}</p>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => updateQty(item.id, -1)}
                        className="w-6 h-6 flex items-center justify-center font-bold text-sm"
                        style={{ border: '1px solid #1E3A6E', color: '#FFFFFF', backgroundColor: 'transparent' }}
                      >−</button>
                    ...
                    <button onClick={() => removeItem(item.id)} className="text-xs hover:opacity-60 ml-1" style={{ color: '#3D5A99' }}>✕</button>
                  </div>
                ))
              )}
            </div>
            ...
            {items.length > 0 && (
              <div className="px-5 pt-4 flex flex-col gap-3" style={{ borderTop: '1px solid #1A2E5A', paddingBottom: 'max(1.25rem, env(safe-area-inset-bottom))' }}>
                {/* District selector - optional */}
                <div className="flex flex-col gap-1 relative" ref={dropdownRef}>
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] tracking-widest uppercase" style={{ color: '#8A9DC0' }}>
                      📍 Entregar en...
                    </label>
                    {selectedDistrict && (
                      <button
                  ...
                  <input
                    ...
                    style={{
                      ...
                      border: `1px solid ${selectedDistrict ? '#EC1C23' : '#1E3A6E'}`,
                      color: '#FFFFFF',
                      outline: 'none',
                    }}
                  />
                  ...
                  {showDropdown && filteredZones.length > 0 && (
                    <motion.div
                      ...
                      style={{ backgroundColor: '#162950', border: '1px solid #1E3A6E' }}
                    >
                      {filteredZones.map((z) => (
                        <button
                          key={z.distrito}
                          onClick={() => handleSelectZone(z)}
                          className="w-full text-left px-3 py-2 text-sm hover:bg-opacity-70 transition-colors flex items-center justify-between"
                          style={{ color: '#FFFFFF', borderBottom: '1px solid rgba(30, 58, 110, 0.3)', backgroundColor: 'transparent' }}
                        >
                          <span>{z.distrito}</span>
                          <span style={{ fontSize: '11px', color: '#EC1C23', fontWeight: 'bold' }}>
                  ...
                  {showDropdown && districtInput && filteredZones.length === 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 rounded px-3 py-2 text-xs" style={{ backgroundColor: '#162950', border: '1px solid #1E3A6E', color: '#8A9DC0' }}>
                      Sin resultados
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex justify-between items-center">
                    <span className="text-xs" style={{ color: '#8A9DC0' }}>Subtotal</span>
                  ...
                  {selectedDistrict && deliveryFee !== null && deliveryFee > 0 && (
                    <div className="flex justify-between items-center">
                      <span className="text-xs" style={{ color: '#8A9DC0' }}>Delivery</span>
                      <span className="text-sm font-bold" style={{ color: '#EC1C23' }}>
                        +{currency}{deliveryFee.toFixed(2)}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between items-center pt-1" style={{ borderTop: '1px solid rgba(236,28,35,0.1)' }}>
                    <span className="text-xs font-black" style={{ color: '#8A9DC0' }}>Total</span>
